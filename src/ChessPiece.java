public abstract class ChessPiece {
	...
	String strErrorMsg = "";
	 ...
	 public abstract boolean legalMove(int startRow, int startColumn, int desRow,
	 ...
	 * @return
	 */
	private boolean checkAxisMove(int newRow, int newColumn,
			int[][] playerMatrix) {
	...
	/**
	 * Method for checking the path to the destination and making sure nothing
	 * is in the way
	 * 
	 * @param startRow
	 * @param startColumn
	 ...
	 * @return
	 */
	protected boolean axisMove(int startRow, int startColumn, int desRow,
			int desColumn, int[][] playerMatrix, boolean straightAxis) {
		if (straightAxis) {
			...
			if ((startColumn == desColumn) && (startRow != desRow)) {
				...
				if (desRow < startRow) {
					...
					// Checks each cell between the start row - 1 (since don't
					// need to check the cell it is in) to the destination cell
			...
			if (desRow < startRow && desColumn < startColumn) { // If moved NW
				// The number of cells moved horizontal should equal the number
				// of cells moved vertical
