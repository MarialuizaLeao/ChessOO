...
public class ChessBoardWindow extends ChessBoard implements MouseListener,
	MouseMotionListener {
    ...
    private Rock rockObject = new Rock();
	...
	private String getPlayerMsg() {
		if (hasWon) {
			...
			return "" + strPlayerName[0] + " you are red, " + strPlayerName[1]
	...
	protected void drawExtra(Graphics g) {
		...
		if (isDragging) {
			g.drawImage(imgPlayer[currentPlayer - 1][pieceBeingDragged],
	...
	private void checkMove(int desRow, int desColumn) {
		...
		if (cellMatrix.getPlayerCell(desRow, desColumn) == currentPlayer) {
			...
			case 0:
				legalMove = pawnObject.legalMove(startRow, startColumn, desRow,
						desColumn, cellMatrix.getPlayerMatrix(), currentPlayer);
				break;
			case 1:
				legalMove = rockObject.legalMove(startRow, startColumn, desRow,
						desColumn, cellMatrix.getPlayerMatrix());
				break;
			case 2:
				legalMove = knightObject.legalMove(startRow, startColumn,
						desRow, desColumn, cellMatrix.getPlayerMatrix());
				break;
			case 3:
				legalMove = bishopObject.legalMove(startRow, startColumn,
						desRow, desColumn, cellMatrix.getPlayerMatrix());
				break;
			case 4:
				legalMove = queenObject.legalMove(startRow, startColumn,
						desRow, desColumn, cellMatrix.getPlayerMatrix());
				break;
			case 5:
				legalMove = kingObject.legalMove(startRow, startColumn, desRow,
						desColumn, cellMatrix.getPlayerMatrix());
				break;
			}
		}
		...
		if (legalMove) {
			...
			if (pieceBeingDragged == 0 && (newDesRow == 0 || newDesRow == 7)) {
				...
				JOptionPane digBox = new JOptionPane(
						"Choose the piece to change your pawn into",
						JOptionPane.QUESTION_MESSAGE,
						JOptionPane.OK_CANCEL_OPTION, null, strPieces, "Rock");
				...
				cellMatrix.setPieceCell(newDesRow, newDesColumn,
						pieceBeingDragged);
			}
	...
	private void updatePaintInstructions(int desRow, int desColumn) {
		currentInstruction = new ChessBoardPainter(startRow, startColumn, 1);
		vecPaintInstructions.addElement(currentInstruction);
		currentInstruction = new ChessBoardPainter(desRow, desColumn);
		vecPaintInstructions.addElement(currentInstruction);
	}
		...
		if (!hasWon && !firstTime) {
			...
			if ((x > 60 && x < 430) && (y > 60 && y < 430)) {
				...
				if (cellMatrix.getPlayerCell(startRow, startColumn) == currentPlayer) {
					pieceBeingDragged = cellMatrix.getPieceCell(startRow,
							startColumn);
